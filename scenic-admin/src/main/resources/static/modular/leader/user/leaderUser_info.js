...
AdminInfoDlg.collectData = function () {
    this.set('id').set('account').set('name').set('scenicId').set('password').set('phone').set('userId');
    /*var prochecked = [];
...
AdminInfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/leaderMgr/edit", function (data) {
        Feng.success("修改成功!");
...
AdminInfoDlg.addSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/leaderMgr/add", function (data) {
        Feng.success("添加成功!");
