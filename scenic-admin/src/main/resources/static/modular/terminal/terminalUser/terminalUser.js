...
Admin.openAddAdmin = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/terminalUser/admin_add'     //+Admin.deptId
    });
...
Admin.openChangeAdmin = function () {
    ...
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/terminalUser/terminalUser_edit/' + this.seItem.id
        });
        this.layerIndex = index;
        layer.full(index);
    }
...
Admin.delMgrUser = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var userId = Admin.seItem.id;
            var ajax = new $ax(Feng.ctxPath + "/terminalUser/delete", function () {
                Feng.success("删除成功!");
                Admin.search();
            }, function (data) {
...
$(function () {
    ...
    var table = new BSTable("adminTable", "/terminalUser/list", defaultColunms);
