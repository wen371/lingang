...
(function (factory) {
    ...
    FileInput.prototype = {
        ...
        _initTemplateDefaults: function () {
            ...
            tTagBef1 = tTagBef + '>' + '<input name="pic" style="display: none" />' + '<div class="kv-file-content">\n';
            tTagBef2 = tTagBef + ' title="{caption}">' + '<input name="pic" style="display: none"/>' + '<div class="kv-file-content">\n';
            tTagAft = '</div>{footer}\n</div>\n';
            tGeneric = '{content}\n';
        ...
        _uploadSingle: function (i, isBatch) {
            ...
            fnSuccess = function (data, textStatus, jqXHR) {
                ...
                $.extend(true, params, outData);
                console.log("AAAA  " + ('#' + previewId + ' input:first') + "  " + data.url);
                $('#' + previewId + ' input:first').val(data.url);
        ...
        _setFileDropZoneTitle: function () {
            ...
            if ($zone.find($h.FRAMES).length === 0 && $h.isEmpty(self.defaultPreviewContent)) {
                $zone.prepend('<div class="' + self.dropZoneTitleClass + '">' + '暂无文件上传' + '</div>');
            }
            self.$container.removeClass('file-input-new');
        ...
        _change: function (e) {
            ...
            total = self.$container.find('.kv-preview-thumb').length;
            console.log('total' + total);
            console.log('lnttt' + self.$container.find('.kv-preview-thumb').length);
            //  console.log('tml:' + self.$element.get(0).html());

            // console.log('fuck:' + self.$element.get(0).files.length);
    ...
    $.fn.fileinputLocales.en = {
        ...
        msgFileRequired: '请选择该上传的文件.',
        msgSizeTooSmall: '文件 "{name}" (<b>{size} KB</b>) 太小,上传最低大小为 <b>{minSize} KB</b>.',
        msgSizeTooLarge: '文件 "{name}" (<b>{size} KB</b>) 超出上传大小限制,最大为 <b>{maxSize} KB</b>.',
        msgFilesTooLess: '您必须最少上传 <b>{n}</b> {files} 个文件.',
        msgFilesTooMany: '您已上传 <b>({n})</b> 张,超出最大上传限制<b>{m}</b>张.',
        msgFileNotFound: '文件 "{name}" 不存在!',
        msgFileSecured: '安全限制阻止阅读该文件 "{name}".',
        msgFileNotReadable: '文件 "{name}" 不可读.',
        msgFilePreviewAborted: '文件预览失败 "{name}".',
        msgFilePreviewError: '文件解析发生错误 "{name}",请重新上传.',
        msgInvalidFileName: '无效的文件名 "{name}".',
        msgInvalidFileType: '不支持的文件格式 "{name}". 只能上传 "{types}" 格式的文件.',
        msgInvalidFileExtension: '不支持的文件格式"{name}". 只能上传 "{extensions}" 格式的文件.',
        msgFileTypes: {
            'image': 'image',
            'html': 'HTML',
            'text': 'text',
        ...
        msgUploadAborted: '文件上传已失败',
        msgUploadThreshold: '处理中...',
        msgUploadBegin: '初始化...',
        msgUploadEnd: '已完成',
        msgUploadEmpty: '暂无可上传数据',
        msgUploadError: '出错',
        msgValidationError: '校验出错',
        msgLoading: 'Loading file {index} of {files} &hellip;',
        msgProgress: 'Loading file {index} of {files} - {name} - {percent}% completed.',
        msgSelected: '{n}个文件已被选中',
        msgFoldersNotAllowed: '只能拖拽文件 {n} 文件夹的拖拽是不允许的',
        ...
        msgAjaxError: '{operation}操作失败. 请重试!',
